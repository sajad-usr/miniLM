...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        doc = self.data[idx]
        rnd = random.randint(4, int(self.block_size*7/8))
        doc = doc[:rnd]
        n = random.randint(int(len(doc)/8), int(len(doc)*3/8))
        start = random.randint(0, len(doc)-n)
        end = start + n
        # [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        doc = doc[:start] + self.MASK_CHAR + doc[end:] + self.MASK_CHAR +\
            doc[start:end]
        doc = doc + self.PAD_CHAR * (self.block_size + 1 - len(doc))
        doc = [self.stoi[s] for s in doc]
        x = torch.LongTensor(doc[:-1])
        y = torch.LongTensor(doc[1:])
        return x, y
        ### END YOUR CODE ###
